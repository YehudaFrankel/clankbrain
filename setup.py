...
""")
    ...
    write("tasks/error-lookup.md", """\
# Error Lookup

<!-- Symptom -> root cause -> fix. Saves 10 minutes per repeat error. -->
<!-- Claude reads this when debugging. Add every error you solve. -->
...
## Pre-checks (before writing this plan)
- [ ] Grepped `tasks/regret.md` for keywords -- no rejected approaches re-proposed
- [ ] Read `coding-conventions.md` -- rules quoted before writing code blocks
- [ ] Checked `tasks/velocity.md` for similar past tasks
...
### Why This Will Work
<!-- One sentence: the causal mechanism, not just "because we changed X" -->
...
### Estimate
- **Similar task:** [from velocity.md] or "no match"
- **This estimate:** N sessions

### Risks
<!-- Concrete risks with mitigations. If zero: "None identified." -->

### Challenge
<!-- Strongest argument AGAINST this approach -->
...
""")
    ...
    print("  Created tasks/ (todo.md, lessons.md, decisions.md, errors.md, error-lookup.md, regret.md, skill_scores.md, velocity.md)")
...
> Placeholder -- will be auto-generated by /scan-codebase on first Start Session.
""")
    write(".claude/rules/plan-before-edit.md", f"""\
# Plan Before Edit -- Required for All Code Changes

**RULE: Before editing ANY code file, present a plan and wait for approval.**

This applies to every change -- CSS, JS, HTML, backend, SQL -- no matter how small.
Does NOT apply to: memory files, .claude/ config files, task files.

---

## Before Every Plan -- Regret Check

Before showing any plan, grep `tasks/regret.md` for keywords matching the proposed approach.
- No match: proceed silently.
- Match found: surface it before the plan:
  > This approach is in regret.md: [entry] -- [why rejected]. Proposing alternative.

## Before Every Plan -- Convention Check

Before writing the After block, read `.claude/rules/coding-conventions.md`.
Quote the applicable rule, then write the code against it.
If you cannot quote the rule, you have not checked it.
...
### Problem / Feature
One sentence: what is broken or what needs to be added.

### All Related Functions
Every function touched -- file path and line number, verified with Grep before showing.

### Before (current code)
```
// the lines that will change
```
...
### Why This Will Work
One sentence explaining the mechanism -- not "because we changed X" but WHY the change fixes it.

### Scope / Blast Radius
- **Files touched:** list every file
- **Lines changed:** count from Before/After
- **Affected at runtime:** what breaks if this goes wrong

### Estimate
Check `tasks/velocity.md` for similar past tasks before estimating.
- **Similar task:** [closest match] or "no match"
- **This estimate:** N sessions
...
2. Show the user the actual content (quote it, don't summarize)
